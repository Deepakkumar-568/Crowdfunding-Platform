...
public static class DatabaseSeeder
{
    public static async Task SeedAsync(ApplicationDbContext context)
    {
        ...
        {
            admin.Email = "admin@crowdfunding.com";
            admin.PasswordHash = HashPassword("Password123");
            context.Users.Update(admin);
        }

        var founder = context.Users.FirstOrDefault(u => u.Username == "founder");
        if (founder == null)
        {
            founder = new User
            {
            ...
            await context.Users.AddAsync(founder);
        }
        else
        {
            founder.Email = "founder@crowdfunding.com";
            founder.PasswordHash = HashPassword("Password123");
            context.Users.Update(founder);
        }

        var investor = context.Users.FirstOrDefault(u => u.Username == "investor");
        if (investor == null)
        {
            investor = new User
            {
        ...
        {
            investor.Email = "investor@crowdfunding.com";
            investor.PasswordHash = HashPassword("Password123");
            context.Users.Update(investor);
        }

        await context.SaveChangesAsync();

        // 2. Seed Startups & Team Members
